...
module.exports = class GeezConverter extends Converter {
    ...
    convert($ascii_number) {
        let $number = this.prepareForConversion($ascii_number),
            $length = $number.length,
            $result = Converter.EMPTY_CHARACTER;
    ...
    prepareForConversion($ascii_number) {
        ...
        let $length = $validated_number.length;

        let $number = this.prependSpaceIfLengthIsEven($validated_number, $length);
    ...
    parseEachTwoCharactersBlock($number, $index, $length) {
        let $geez_number = this.getGeezNumberOfTheBlock($number, $index);

        let $bet = this.getBet($length, $index);

        let $geez_separator = this.getGeezSeparator($bet);
    ...
    getGeezNumberOfTheBlock($number, $index) {
        let $block = this.getBlock($number, $index);

        let $tenth = Number.isNaN(parseInt($block[0])) ? 0 : parseInt($block[0]);
        let $once = parseInt($block[1]);
     ...
     */
    getBet($length, $index) {
        let $reverse_index = (($length - 1) - $index);
    ...
    shouldRemoveGeezNumberBlock($block, $separator, $index) {
        return this.isOneHundred($block, $separator) ||
    ...
    isOneHundred($block, $separator) {
        return this.isGeezNumberHundred($separator) &&
